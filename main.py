...
CANVAS_SIZE = (854, 480)  # default canvas for title/overlay preview images
...
def load_clip_resized(path, target_height, start=None, end=None):
...
with tabs[2]:
    ...
    title_fontsize = st.slider("Title text size", 20, 120, 60, key="title_fontsize")
    ...
    overlay_fontsize = st.slider("Overlay text size", 14, 80, 40, key="overlay_fontsize")
...
with tabs[5]:
    ...
    if render_btn:
        if not st.session_state.timeline:
            ...
            try:
                target_height = 720 if export_quality.startswith("720") else 480
                render_canvas = (int(target_height * 16 / 9), target_height)
                ...
                with st.spinner("Rendering... this can take a minute depending on clip length."):
                    ...
                    if add_title and title_text.strip():
                        title_img = make_title_image(
                            title_text, FONT_OPTIONS[title_font], title_fontsize, title_color, title_bg,
                            size=render_canvas,
                        )
                    ...
                    for item in st.session_state.timeline:
                        clip = load_clip_resized(item["path"], target_height, item["start"], item["end"])
                        clips.append(clip)

                    main_clip = concatenate_videoclips(clips, method="compose")

                    # 3. Effects (moviepy 2.x: effect classes applied via with_effects)
                    video_effects = []
                    if fx_speed != 1.0:
                        video_effects.append(vfx.MultiplySpeed(fx_speed))
                    if fx_bw:
                        video_effects.append(vfx.BlackAndWhite())
                    if fx_fadein:
                        video_effects.append(vfx.FadeIn(1))
                    if fx_fadeout:
                        video_effects.append(vfx.FadeOut(1))
                    if video_effects:
                        main_clip = main_clip.with_effects(video_effects)

                    # 4. Text overlay across whole trailer
                    if add_overlay and overlay_text.strip():
                        overlay_img = make_overlay_image(
                            overlay_text, FONT_OPTIONS[overlay_font], overlay_fontsize, overlay_color,
                            overlay_position, size=render_canvas,
                        )
                        overlay_clip = ImageClip(overlay_img).with_duration(main_clip.duration)
                    ...
                    if music_file is not None:
                        ...
                        if bg_audio.duration < main_clip.duration:
                            bg_audio = bg_audio.with_effects([afx.AudioLoop(duration=main_clip.duration)])
                        else:
                            bg_audio = bg_audio.subclipped(0, main_clip.duration)
                        bg_audio = bg_audio.with_effects([afx.MultiplyVolume(music_volume)])
                        audio_tracks.append(bg_audio)

                    if audio_tracks:
                        final_audio = CompositeAudioClip(audio_tracks).with_duration(main_clip.duration)
